...
def construct_headers(model_type: str, credentials: DataRobotDeploymentCredentials, accept_csv_response: bool=False, is_chat_api_request: bool=False) -> dict:
...
def construct_payload(input_type: str, column_names: list[str], column_values: list[str], association_id_column: str | None) -> bytes:
    """Construct the payload expected by the deployment."""
    if not (isinstance(column_names, list) and isinstance(column_values, list) and (len(column_names) == len(column_values))):
...
def construct_payload_from_dict(data: dict[str, Any], input_type: str, association_id_column: str | None) -> bytes:
...
def parse_response_data(response_data: dict | pd.DataFrame, model_type: str, target_column_name: str) -> Any:
...
def is_valid_response_with_extra_output(response_data: dict) -> bool:
    ...
    if 'data' in response_data and EXTRA_MODEL_OUTPUT_KEY in response_data['data'][0] and isinstance(response_data['data'][0][EXTRA_MODEL_OUTPUT_KEY], dict) and response_data['data'][0].get(EXTRA_MODEL_OUTPUT_KEY):
        return True
    return False

def parse_documents_from_response_data(response_data: dict | Any, model_type: str, target_column_name: str) -> list[Document]:
    ...
    if model_type == 'UNSTRUCTURED':
        ...
        if is_valid_response_with_extra_output(response_data) and METADATA_COLUMN in response_data['data'][0][EXTRA_MODEL_OUTPUT_KEY]:
            metadata_column_contents = response_data['data'][0][EXTRA_MODEL_OUTPUT_KEY][METADATA_COLUMN]
    ...
    return [Document(page_content=content, metadata=metadata) for content, metadata in zip(document_list, metadata_list)]

def _parse_metadata(document_list: list[str], metadata_column: dict[str, list[str]] | list[dict[str, str]]) -> list[dict[str, Any]]:
    """
    ...
    for i in range(len(document_list)):
        parsed_metadata = {}
        metadata_to_parse = metadata_column[i] if isinstance(metadata_column, list) else metadata_column
...
def parse_query_embeddings_from_response_data(response_data: dict | Any) -> QueryEmbeddings | None:
    ...
    if isinstance(query_embedding, str):
        try:
            ...
            logger.warning('Query embeddings found for external model, but were not valid json. Embeddings will not be reported.')
