...
FinalPromptType: TypeAlias = str | GPTFinalPromptType | BedrockFinalPromptType | GoogleFinalPromptType | None
...
class Tool(BaseModel):
    ...
    def to_bedrock_tool_dict(self) -> dict[str, Any]:
        ...
        return {'name': self.name, 'description': self.description, 'inputSchema': {'json': self.parameters}}

    @staticmethod
    def from_openai_tool_dict(tool_dict: dict[str, Any]) -> Tool:
        return Tool(name=tool_dict['function']['name'], description=tool_dict['function']['description'], parameters=tool_dict['function']['parameters'])
...
class LanguageModelInvalidRequestError(LanguageModelError):
    ...
    def __init__(self, msg: str=gettext_noop("The LLM has received an invalid request. Possible causes include exceeding the LLM's context length or an invalid parameter in the chat completion request. Try reducing the user or system prompt, max completion length, or vector database retrieval limits. If calling the chat API of a DataRobot deployment, check the syntax of the parameters in the request. If the issue persists, contact DataRobot."), internal_error_msg: str=gettext_noop('No detailed internal error message was supplied.'), *args: Any, **kwargs: Any):
        self.internal_error_msg = internal_error_msg
        combined_msg = gettext('{msg} Details: {internal_error_msg}').format(msg=gettext(msg), internal_error_msg=gettext(internal_error_msg))
...
class LanguageModelInterface:
    ...
    @abstractmethod
    async def submit_prompt(self, messages: Sequence[ChatCompletionMessageParam], llm_settings: dict | None=None, docs: Sequence[Document] | None=None, model_id: str | None=None) -> BaseLanguageModelResult:
        pass
...
class LanguageModel(LanguageModelInterface):
    ...
    def __init__(self, credentials: BaseModel | None=None) -> None:
        self.collected_result_metadata = BaseResultMetadata(output_token_count=0, input_token_count=0, total_token_count=0, latency_milliseconds=0)
    ...
    @abstractmethod
    def _submit_prompt_with_response_streaming(self, messages: Sequence[ChatCompletionMessageParam], llm_settings: dict, docs: Sequence[Document] | None=None, model_id: str | None=None) -> AsyncIterator[ChatCompletionChunk]:
    ...
    async def submit_prompt(self, messages: Sequence[ChatCompletionMessageParam], llm_settings: dict | None=None, docs: Sequence[Document] | None=None, model_id: str | None=None) -> BaseLanguageModelResult:
        ...
        if self.enable_test_mode:
            ...
            llm_result = await self._submit_prompt(messages=messages, llm_settings=llm_settings, docs=docs, model_id=model_id)
        logger.bind(model_class_name=self.__class__.__name__, model_module_name=self.__module__.split('.')[-1], input_token_count=llm_result.result_metadata.input_token_count, output_token_count=llm_result.result_metadata.output_token_count, latency_milliseconds=llm_result.result_metadata.latency_milliseconds).info('LLM prompt completed')
    ...
    async def submit_prompt_with_response_streaming(self, messages: Sequence[ChatCompletionMessageParam], llm_settings: dict | None=None, docs: Sequence[Document] | None=None, model_id: str | None=None, final_chunk_hook: Callable[[ChatCompletionChunk], None] | None=None) -> AsyncIterator[ChatCompletionChunk]:
        """
        Submit a prompt to a DataRobot deployment or an external LLM service,
        ...
        if self.enable_test_mode:
            # Return a mock response that echoes the contents of the request.
            response_stream = generate_test_completion_streaming(messages=messages, llm_settings=llm_settings, docs=docs, model_id=model_id or 'llm-blueprint')
        else:
            # Use the LLM client to submit the prompt to the LLM service.
            response_stream = self._submit_prompt_with_response_streaming(messages, llm_settings=llm_settings, docs=docs, model_id=model_id)
    ...
    async def ping(self) -> bool:
        ...
        """
        messages: list[ChatCompletionMessageParam] = [ChatCompletionUserMessageParam(content='Why is the sky blue?', role='user')]
        result = await self.submit_prompt(messages, llm_settings={})
        return bool(result.result_text)

def generate_test_llm_result(messages: Sequence[ChatCompletionMessageParam], llm_settings: dict | None=None, docs: Sequence[Document] | None=None) -> BaseLanguageModelResult:
    """Generate a mock LLM result to be used in `LanguageModel.enable_test_mode`."""
    return BaseLanguageModelResult(result_text=f"This is a mock LLM response. Prompt: '{messages[-1]['content']}'. Settings: {llm_settings}. Number of messages: {len(messages)}. Number of documents: {(len(docs) if docs else 0)}.", result_metadata=BaseResultMetadata(input_token_count=10, output_token_count=20, total_token_count=30, latency_milliseconds=1000))

async def generate_test_completion_streaming(messages: Sequence[ChatCompletionMessageParam], llm_settings: dict | None=None, docs: Sequence[Document] | None=None, model_id: str='llm-blueprint', chunk_delay_seconds: float=0.1) -> AsyncIterator[ChatCompletionChunk]:
    ...
    yield ChatCompletionChunk(id=completion_id, choices=[StreamingChoice(delta=ChoiceDelta(role='assistant', content=None), finish_reason=None, index=0)], created=completion_timestamp, model=model_id, object='chat.completion.chunk')
    # Yield the chunk with the response text.
    await asyncio.sleep(chunk_delay_seconds)
    yield ChatCompletionChunk(id=completion_id, choices=[StreamingChoice(delta=ChoiceDelta(role='assistant', content=result_text), finish_reason=None, index=0)], created=completion_timestamp, model=model_id, object='chat.completion.chunk')
    # Yield the final chunk with the finish reason.
    await asyncio.sleep(chunk_delay_seconds)
    yield ChatCompletionChunk(id=completion_id, choices=[StreamingChoice(delta=ChoiceDelta(role='assistant', content=None), finish_reason='stop', index=0)], created=completion_timestamp, model=model_id, object='chat.completion.chunk')