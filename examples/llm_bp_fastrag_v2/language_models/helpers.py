...
def prune_docs(docs: list[Document], docs_token_budget: int, tokenizer: PreTrainedTokenizerBase | None=None) -> tuple[list[Document], int]:
...
def get_excess_token_budget(context_size: int | None, output_tokens: int | None, system_prompt: str | None, user_prompt: str, tokenizer: PreTrainedTokenizerBase | None=None) -> int:
...
def get_tool_settings(llm_settings: dict[str, Any]) -> dict[str, Any]:
    ...
    if llm_settings.get('tools'):
        ...
        if len(required_tools) > 1:
            ...
            openai_settings['tool_choice'] = {'type': 'function', 'function': {'name': required_tools[0].name}}
        else:
            openai_settings['tool_choice'] = 'auto'
    return openai_settings

def prepend_system_message_to_messages(messages: Sequence[ChatCompletionMessageParam], system_prompt: str, docs: Sequence[Document]) -> list[ChatCompletionMessageParam]:
    """
...
def get_settings_and_system_prompt(llm_settings: dict[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    if 'max_completion_length' in llm_settings:
        llm_settings['max_completion_tokens'] = llm_settings.pop('max_completion_length')
    ...
    # OpenAI (and Gemini via LiteLLM) requires logprobs=True when top_logprobs is set.
    # Coerce it automatically so users don't need to set both fields explicitly.
    if custom_model_settings.get('top_logprobs') is not None and (not custom_model_settings.get('logprobs')):
        custom_model_settings['logprobs'] = True
