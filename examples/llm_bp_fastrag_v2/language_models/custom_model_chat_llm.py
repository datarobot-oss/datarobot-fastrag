...
class CustomModelChatLLMPromptError(CustomModelChatLLMError):
    ...
    @property
    def user_message(self) -> str:
        return gettext('Custom model failed to execute chat completion prompt.\nError: {error_message}.\n{error_details}').format(error_message=self.error_message, error_details=self.error_details)

class CustomModelChatLLM(LanguageModel):
    """LLM implemented as deployed custom model."""

    def __init__(self, datarobot_endpoint: str | None=None, authorization_header: str | None=None, user_agent_header: str | None=None, playground_context: BasePlaygroundContext | None=None):
        super().__init__()
    ...
    async def _submit_prompt(self, messages: Sequence[ChatCompletionMessageParam], llm_settings: dict, docs: Sequence[Document] | None=None, model_id: str | None=None) -> BaseLanguageModelResult:
        ...
        with logger.contextualize(custom_model_id=custom_model_id):
            ...
            url = os.path.join(self.datarobot_endpoint, f'genai/agents/fromCustomModel/{custom_model_id}/chat/')
            if system_prompt:
            ...
            if self.playground_context:
                payload['tracingContext'] = {'entityId': self.playground_context.use_case_id, 'entityType': 'use_case', 'attributes': {'chat_prompt_id': self.playground_context.chat_prompt_id, 'playground_id': self.playground_context.playground_id}}
            async with ClientSession(headers=self.headers, timeout=ClientTimeout(total=TIMEOUT_INDIVIDUAL_REQUEST)) as session:
                time_start = time()
                ...
                if moderation_lib_results:
                    guard_configurations = await get_guard_configuration_for_custom_model(session, self.datarobot_endpoint, custom_model_id)
                    metrics = get_metrics_metadata(guard_configurations, response_data['datarobot_moderations'])
            result_metadata = BaseResultMetadata(output_token_count=usage.get('completion_tokens', 0), input_token_count=usage.get('prompt_tokens', 0), total_token_count=usage.get('total_tokens', 0), latency_milliseconds=latency_milliseconds, pipeline_interactions=response_data.get('pipeline_interactions', None), metrics=metrics)
            return BaseLanguageModelResult(result_text=response_data['choices'][0]['message']['content'], result_metadata=result_metadata)
    ...
    async def _chat_completion_async(self, session: ClientSession, url: str, payload: dict) -> dict[str, Any]:
        ...
        if not response.ok or not response.headers.get('Location'):
            ...
            logger.bind(response_status=status, response_text=error).error('Custom model chat failed')
            raise CustomModelChatLLMSetupError(error)
        chat_completion_location = await self._wait_for_async_resolution(session, response.headers['Location'])
    ...
    async def _wait_for_async_resolution(self, session: ClientSession, async_location: str, max_wait: int=MAX_WAIT, polling_interval: float=POLLING_INTERVAL) -> str:
        """Wait for async response to resolve."""
        ctx_logger = logger.bind(async_location=async_location, max_wait=max_wait, polling_interval=polling_interval)
        start_time = time()
        while time() < start_time + max_wait:
            ...
            if data['status'] in {'ERROR', 'ABORTED'}:
                ...
                raise CustomModelChatLLMSetupError(data.get('message') or gettext('Failed to run custom model in a codespace.'))
            await asyncio.sleep(polling_interval)
        ctx_logger.error('Timeout waiting for chat completion async job to resolve')
        raise TimeoutError(gettext('Client timed out waiting for chat completion async job to resolve'))