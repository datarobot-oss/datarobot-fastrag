...
from vector_database.exception import EmbeddingModelLoadingError
from vector_database.openai_embeddings import OpenAIEmbeddingClient
...
def _detect_tokenizer_architecture_mismatch(model_name_or_path: str, architectures: list[str], tokenizer_class: str) -> bool:
...
def patch_sentence_transformer() -> None:
    ...
    def forward(cls: Any, features: dict[str, Any]) -> dict[str, Any]:
        ...
        if _detect_tokenizer_architecture_mismatch(cls.auto_model.config._name_or_path, cls.auto_model.config.architectures, cls.auto_model.config.tokenizer_class):
...
def get_sentence_transformer(model_path: str) -> sentence_transformers.SentenceTransformer:
    ...
    return sentence_transformers.SentenceTransformer(model_path, cache_folder=model_path, backend='onnx', model_kwargs={'provider': onnx_provider, 'file_name': 'model_quantized.onnx'})
EmbeddingClient = sentence_transformers.SentenceTransformer | CustomModelEmbeddingClient | OpenAIEmbeddingClient

def get_embedder(client: EmbeddingClient, batch_size: int=DEFAULT_BATCH_SIZE) -> Embeddings:
    try:
        if isinstance(client, sentence_transformers.SentenceTransformer):
            ...
            return DataRobotEmbeddings(client=client, encode_kwargs={'batch_size': batch_size, 'normalize_embeddings': True, 'show_progress_bar': False, 'device': torch.device(get_device()).type})
        elif isinstance(client, CustomModelEmbeddingClient):
            return CustomModelEmbeddings(client=client)
        elif isinstance(client, OpenAIEmbeddingClient):
            return OpenAIModelEmbeddings(client=client)
        else:
...
class DataRobotEmbeddings(Embeddings):
    ...
    def __init__(self, client: sentence_transformers.SentenceTransformer, encode_kwargs: dict[str, Any] | None=None, **kwargs: Any):
        self.client = client
...
class ExternalEmbeddings(Embeddings, ABC):

    def __init__(self, client: CustomModelEmbeddingClient | OpenAIEmbeddingClient, **kwargs: Any):
        self.client = client
...
class CustomModelEmbeddings(ExternalEmbeddings):
    """Langchain interface to DataRobot embedding models."""

class OpenAIModelEmbeddings(ExternalEmbeddings):
    """Langchain interface to OpenAI compatible embedding models."""