...
class BaseChunkRepository(ABC):
    ...
    def _get_metadata_columns_to_include(self, available_columns: Iterable[str]) -> list[str]:
        metadata_columns = [column_name for column_name in available_columns if column_name not in self.RESERVED_METADATA_FIELDS]
    ...
    async def retrieve_text_chunks(self, indices: Sequence[int]) -> list[Document]:
        ...
        async with self.text_chunks_file() as text_file:
            for index, metadata_for_index in zip(indices, metadata_for_indices):
                ...
                chunk_bytes = await asyncio.to_thread(decompress_chunk, chunk_bytes, chunk_compression_type)
                documents.append(Document(page_content=chunk_bytes.decode('utf-8'), metadata=metadata_for_index))
        self.logger.debug('Finished retrieving text chunks and preparing Document objects to return')
        return documents
...
class DataFrameBaseChunkRepository(BaseChunkRepository):
    ...
    async def _get_metadata_for_indices(self, indices: Sequence[int]) -> list[dict[str, Any]]:
        ...
        for index in indices:
            ...
            metadata = {column: cast_numpy_to_native(metadata_dataframe.at[index, column]) for column in metadata_column}
...
class SQLBaseChunkRepository(BaseChunkRepository):
    ...
    def _chunk_sizes(self) -> np.ndarray:
        ...
        with self.sql_connection() as connection:
            s = select(self.sql_table.c[MetadataColumnNames.chunk_size.value]).order_by(MetadataColumnNames.chunk_id.value)
    ...
    def __get_metadata_for_indices(self, indices: Sequence[int]) -> list[dict[str, Any]]:
        ...
        columns_to_include = self._get_metadata_columns_to_include([c.name for c in self.sql_table.columns])
        data = {}
        with self.sql_connection() as connection:
            ...
            for inices_batch in self.batch(indices, batch_size=SQLITE_MAX_VARIABLE_NUMBER):  # type: ignore[call-overload]
                s = select(self.sql_table.c[*columns_to_include,]).where(self.sql_table.c[MetadataColumnNames.chunk_id.value].in_(inices_batch))
            ...
            try:
                match operator:
                    ...
                    case _:
                        ...
                        # accidentally
                        raise ValueError(gettext('The metadata filter contains an unsupported operator. Supported operators are: {supported_operators}').format(supported_operators='$eq, $ne, $gt, $gte, $lt, $lte, $in, $nin, $contains, $not_contains'))
            except sqlalchemy.exc.ArgumentError:
                raise ValueError(gettext('Invalid metadata filter. One of the values had an incompatible type.'))
    ...
    def _build_filter_expression(self, filters: dict | None) -> Any:
        ...
        for field, value in filters.items():
            ...
            if field == MetadataFilterOperators.AND.value:
                conditions.append(sqlalchemy.and_(*[self._build_filter_expression(f) for f in value]))
            elif field == MetadataFilterOperators.OR.value:
                conditions.append(sqlalchemy.or_(*[self._build_filter_expression(f) for f in value]))
            elif isinstance(value, dict):
    ...
    def _get_indices_matching_filters(self, filters: dict[str, Any] | None) -> np.ndarray:
        ...
        with self.sql_connection() as connection:
            s = select(self.sql_table.c[MetadataColumnNames.chunk_id.value]).where(self._build_filter_expression(filters))
            try:
