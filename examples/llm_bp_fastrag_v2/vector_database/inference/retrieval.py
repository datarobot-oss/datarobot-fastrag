...
def maximal_marginal_relevance(query_embedding: np.ndarray | list[float], embedding_list: np.ndarray | list[list[float]], lambda_mult: float, k: int) -> list[int]:
...
class FaissVectorStore(VectorStore):
    ...
    def __init__(self, index: faiss.Index, embedder: Embeddings, chunk_repository: BaseChunkRepository, relevance_score_fn: Callable=_default_relevance_score_fn):
    ...
    async def _search(self, query_embeddings: np.ndarray, search_k: int, params: faiss.SearchParameters | None) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        if isinstance(self.index, faiss.IndexBinaryFlat):
            ...
            vectors = np.asarray([self.index.reconstruct(i) for i in indices[indices != -1].tolist()])
            ...
            scores, indices, vectors = self.index.search_and_reconstruct(query_embeddings, search_k, params=params)
        logger.debug('Finished searching in the VDB')
        return (scores, indices, vectors, query_embeddings)

    async def _retrieve_and_filter_documents(self, scores: list[float], indices: list[int], vectors: list[QueryEmbeddings], filter: dict[str, Any] | None, add_neighbor_chunks: bool) -> list[Document]:
        ...
        ids_to_scores_and_vectors_map = {int(i): (self.relevance_score_fn(score), vector) for i, score, vector in zip(indices, scores, vectors) if i != -1}
        doc_ids = list(ids_to_scores_and_vectors_map.keys())
        if add_neighbor_chunks:
            doc_ids = [*dict.fromkeys([j for i in doc_ids for j in range(i - 1, i + 2) if j >= 0 and j <= self.index.ntotal - 1])]
        docs = await self.chunk_repository.retrieve_text_chunks(doc_ids)
        ids_to_consider = (await self.chunk_repository.get_indices_matching_filters(filter)).tolist() if filter else None
        filtered_docs = []
    ...
    async def search(self, query: str, k: int=DEFAULT_K, filter: dict[str, Any] | None=DEFAULT_FILTER, add_neighbor_chunks: bool=DEFAULT_ADD_NEIGHBOR_CHUNKS, retrieval_mode: RetrievalMode=RetrievalMode.SIMILARITY, maximal_marginal_relevance_lambda: float=0.5, **kwargs: Any) -> SearchResult:
        ...
        if retrieval_mode == RetrievalMode.SIMILARITY:
            scores, indices, vectors, query_embeddings = await self._search(query_embeddings, k, params)
        elif retrieval_mode == RetrievalMode.MAXIMAL_MARGINAL_RELEVANCE:
            scores, indices, vectors, output_query_embeddings = await self._search(query_embeddings, 5 * k, params)
            embeddings = np.unpackbits(vectors, axis=-1).astype(int) * 2 - 1
            selected_indices = maximal_marginal_relevance(query_embeddings, embeddings[0], lambda_mult=maximal_marginal_relevance_lambda, k=k)
        ...
        documents = await self._retrieve_and_filter_documents(scores[0].tolist(), indices[0].tolist(), vectors[0].tolist(), filter, add_neighbor_chunks)
    ...
    async def add_neighbor_chunks(self, docs: list[Document], filter: dict[str, Any] | None=DEFAULT_FILTER, **kwargs: Any) -> list[Document]:
        ...
        return await self._retrieve_and_filter_documents(scores, indices, vectors, filter, add_neighbor_chunks=True)
...
async def _make_call_to_custom_vdb(session: ClientSession, prediction_api_url: str, payload: bytes) -> ClientResponse:
    try:
...
class CustomModelVectorDatabaseAsVectorStore(VectorStore):
    ...
    def __init__(self, prediction_api_url: str, datarobot_key: str | None, authorization_header: str, model_type: str, input_type: str, prompt_column_name: str, target_column_name: str, association_id_column: str | None, error_handler: BaseCustomModelVectorDatabaseErrorHandler | None=None, identity_token_loader: Callable | None=None):
    ...
    async def search(self, query: str, k: int=DEFAULT_K, filter: dict[str, Any] | None=DEFAULT_FILTER, add_neighbor_chunks: bool=DEFAULT_ADD_NEIGHBOR_CHUNKS, retrieval_mode: RetrievalMode=RetrievalMode.SIMILARITY, maximal_marginal_relevance_lambda: float=0.5, **kwargs: Any) -> SearchResult:
        ...
        credentials = DataRobotDeploymentCredentials(datarobot_key=self.datarobot_key, authorization_header=self.authorization_header)
        query_embeddings = None
        headers = self.construct_headers(credentials)
        payload_dict = {self.prompt_column_name: query, 'k': k, 'metadata_filter': self.convert_metadata_filter_to_json(metadata_filter=filter), 'add_neighbor_chunks': add_neighbor_chunks}
        payload = construct_payload_from_dict(payload_dict, self.input_type, self.association_id_column)
        try:
            async with aiohttp.ClientSession(headers=headers, raise_for_status=True, timeout=ClientTimeout(total=60)) as session:
            ...
            if self.error_handler is not None:
                ...
                msg = gettext('Vector database request returned an error. Try again, and if that does not resolve the issue, send the prompt to the associated LLM blueprint and a more detailed error will be returned in the playground.')
        ...
        docs = parse_documents_from_response_data(response_data, self.model_type, self.target_column_name)
        query_embeddings = parse_query_embeddings_from_response_data(response_data)
        return (docs, query_embeddings)

    def convert_metadata_filter_to_json(self, metadata_filter: dict[str, Any] | None=DEFAULT_FILTER) -> dict[str, Any] | str | None:
        if metadata_filter:
...
class BaseRetriever(ABC):
    ...
    def __init__(self, vector_store: VectorStore, language_model: LanguageModelInterface, llm_settings: dict, number_of_documents_to_retrieve: int, add_neighbor_chunks: bool=False, prompt_template: str | None=None, retrieval_mode: RetrievalMode=DEFAULT_RETRIEVAL_MODE, maximal_marginal_relevance_lambda: float=DEFAULT_MAXIMAL_MARGINAL_RELEVANCE_LAMBDA):
    ...
    @abstractmethod
    async def _get_docs_and_query(self, query: str, history: Sequence[ChatCompletionMessageParam], metadata_filter: dict[str, Any] | None=None) -> DocsQueryResult:
        pass

    async def get_docs_and_query(self, query: str, history: Sequence[ChatCompletionMessageParam], metadata_filter: dict[str, Any] | None=None) -> DocsQueryResult:
        ...
        """
        top_k = self.number_of_documents_to_retrieve * 3 if self.add_neighbor_chunks else self.number_of_documents_to_retrieve
        slice = 2
        while True:
            try:
                documents, query, history, query_embeddings = await self._get_docs_and_query(query, history, metadata_filter)
                break
...
class SingleLookUpRetriever(BaseRetriever):
    ...
    async def _get_docs_and_query(self, query: str, history: Sequence[ChatCompletionMessageParam], metadata_filter: dict[str, Any] | None=None) -> DocsQueryResult:
        documents, query_emb = await self.vector_store.search(add_neighbor_chunks=self.add_neighbor_chunks, query=query, k=self.number_of_documents_to_retrieve, filter=metadata_filter, retrieval_mode=self.retrieval_mode, maximal_marginal_relevance_lambda=self.maximal_marginal_relevance_lambda)
...
class ConversationalRetriever(BaseRetriever):
    ...
    async def _get_docs_and_query(self, query: str, history: Sequence[ChatCompletionMessageParam], metadata_filter: dict[str, Any] | None=None) -> DocsQueryResult:
        ...
        language_model_result = await self.language_model.submit_prompt(messages=messages, llm_settings=self.llm_settings)
        ...
        if language_model_result.result_text is None:
            ...
            single_look_up_retriever = SingleLookUpRetriever(vector_store=self.vector_store, language_model=self.language_model, llm_settings=self.llm_settings, number_of_documents_to_retrieve=self.number_of_documents_to_retrieve, add_neighbor_chunks=self.add_neighbor_chunks, retrieval_mode=self.retrieval_mode, maximal_marginal_relevance_lambda=self.maximal_marginal_relevance_lambda)
            documents, query, new_history, query_emb = await single_look_up_retriever._get_docs_and_query(query=query, history=history, metadata_filter=metadata_filter)
        else:
            # Some LLMs echo back the "STANDALONE QUERY:" completion marker from the prompt
            # template. Strip it so the marker never leaks into the VDB search or the trace.
            result_text = language_model_result.result_text.strip().removeprefix('STANDALONE QUERY:').strip()
            documents, query_emb = await self.vector_store.search(query=result_text, k=self.number_of_documents_to_retrieve, add_neighbor_chunks=self.add_neighbor_chunks, filter=metadata_filter, retrieval_mode=self.retrieval_mode, maximal_marginal_relevance_lambda=self.maximal_marginal_relevance_lambda)
            query = result_text
...
class MultiStepRetriever(BaseRetriever):
    ...
    async def _get_docs_and_query(self, query: str, history: Sequence[ChatCompletionMessageParam], metadata_filter: dict[str, Any] | None=None) -> DocsQueryResult:
        conversational_retriever = ConversationalRetriever(vector_store=self.vector_store, language_model=self.language_model, llm_settings=self.llm_settings, number_of_documents_to_retrieve=self.number_of_documents_to_retrieve, add_neighbor_chunks=False, retrieval_mode=self.retrieval_mode, maximal_marginal_relevance_lambda=self.maximal_marginal_relevance_lambda)
        documents, rewritten_query, new_history, query_emb = await conversational_retriever._get_docs_and_query(query=query, history=history, metadata_filter=metadata_filter)
        ...
        if language_model_result.result_text:
            ...
            add_neighbors_in_search = self.add_neighbor_chunks and (not can_add_neighbors_separately)
            ...
            tasks = [self.vector_store.search(query=query, k=self.number_of_documents_to_retrieve, add_neighbor_chunks=add_neighbors_in_search, filter=metadata_filter, retrieval_mode=self.retrieval_mode, maximal_marginal_relevance_lambda=self.maximal_marginal_relevance_lambda) for query in search_queries if len(query) > 0]
            ...
            if add_neighbors_after_search:
                final_documents = await self.vector_store.add_neighbor_chunks(final_documents, metadata_filter, apply_access_control_list=True)
        else:
...
VECTOR_DATABASE_RETRIEVER_MAP: Mapping[str, Type[BaseRetriever]] = {'SINGLE_LOOKUP_RETRIEVER': SingleLookUpRetriever, 'CONVERSATIONAL_RETRIEVER': ConversationalRetriever, 'MULTI_STEP_RETRIEVER': MultiStepRetriever}

def get_retriever(retriever_name: str | None, vector_store: VectorStore, language_model: LanguageModelInterface | Any, llm_settings: dict, number_of_documents_to_retrieve: int, add_neighbor_chunks: bool, retrieval_mode: RetrievalMode=DEFAULT_RETRIEVAL_MODE, maximal_marginal_relevance_lambda: float=DEFAULT_MAXIMAL_MARGINAL_RELEVANCE_LAMBDA) -> BaseRetriever:
    ...
    retriever = retriever_class(vector_store=vector_store, language_model=language_model, llm_settings=llm_settings, number_of_documents_to_retrieve=number_of_documents_to_retrieve, add_neighbor_chunks=add_neighbor_chunks, retrieval_mode=retrieval_mode, maximal_marginal_relevance_lambda=maximal_marginal_relevance_lambda)
    return retriever
