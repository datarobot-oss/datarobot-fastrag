...
DATAROBOT_DOME_STAGE_TO_BUZOK_STAGE_MAP = {
    GuardStage.PROMPT: BasePipelineStage.PROMPT_PIPELINE,
    GuardStage.RESPONSE: BasePipelineStage.RESPONSE_PIPELINE,
}
# The API defines also PII (deprecated) and userModel (defined in the lib as ootb type with
# OOTBType.USER_MODEL).
API_GUARD_TYPE_TO_MODERATION_LIB_GUARD_TYPE: Dict[str, GuardType] = {
    "guardModel": GuardType.MODEL,
    "nemo": GuardType.NEMO_GUARDRAILS,
    "ootb": GuardType.OOTB,
}


def get_metrics_metadata(
    guard_configurations: list[Dict[str, Any]], moderation_lib_results: Dict[str, Any]
) -> list[BaseMetricMetadata]:
    ...
    for guard_configuration in guard_configurations:
        ...
        for stage in stages:
            ...
            metric_result_key = get_metric_column_name(
                guard_type=guard_type,
                ootb_type=ootb_guard_type,
                stage=stage,
                model_guard_target_name=model_guard_target_name,
            )
            is_metric_value_present = metric_result_key in moderation_lib_results
            execution_status = (
                BaseExecutionStatus.COMPLETED
                if is_metric_value_present
                else BaseExecutionStatus.ERROR
            )
            error_message = None
            ...
            metric = BaseMetricMetadata(
                name=_get_metric_name(guard_name, stage, stages),
                value=moderation_lib_results.get(metric_result_key),
                stage=DATAROBOT_DOME_STAGE_TO_BUZOK_STAGE_MAP[stage],
                execution_status=execution_status,
                error_message=error_message,
                custom_model_guard_id=get_guard_id(
                    guard_name, guard_configuration["type"], stage, ootb_guard_type
                ),
            )
...
async def get_guard_configuration_for_custom_model(
    session: ClientSession, datarobot_endpoint: str, custom_model_id: str
) -> list[Dict[str, Any]]:
    response = await session.get(
        os.path.join(datarobot_endpoint, f"customModels/{custom_model_id}/")
    )
    ...
    response = await session.get(
        os.path.join(datarobot_endpoint, "guardConfigurations/"), params=params
    )
    guard_configurations = await response.json()
    return decamelize(guard_configurations["data"])


def _does_guard_provide_metric_result(
    guard_type: GuardType, guard_configuration: Dict[str, Any]
) -> bool:
    result = True
...
def get_guard_id(name: str, guard_type: str, stage: str, ootb_type: str | None = None) -> str:
