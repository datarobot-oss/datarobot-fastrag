...
async def make_prompt_call_to_custom_llm(
    session: ClientSession, inference_api_url: str, payload: bytes
) -> ClientResponse:
    response = await session.post(inference_api_url, data=payload)
    if response.status in [502, 503, 504]:
        logger.bind(base_url=inference_api_url, status_code=response.status).warning(
            DEPLOYMENT_TEMPORARILY_UNAVAILABLE_MESSAGE
        )  # type: ignore[arg-type]
        raise RetryableCustomModelError() from ClientResponseError(
        ...
        raise ClientResponseError(
            ...
            response.history,
            status=response.status,
            message=error_message.get("message", response.reason),
            headers=response.headers,
        )
    else:
...
async def make_openai_chat_api_call_to_custom_llm(
    client: AsyncOpenAI, **kwargs: Any
    ...
    try:
        ...
        if e.status_code in [502, 503, 504]:
            logger.bind(base_url=client.base_url, status_code=e.status_code).warning(
                DEPLOYMENT_TEMPORARILY_UNAVAILABLE_MESSAGE
            )
...
class CustomModelLLM(LanguageModel):
    ...
    def __init__(
        ...
        supports_chat_api: bool = False,
        chat_model_id: str | None = None,
        error_handler: BaseCustomModelLLMErrorHandler | None = None,
    ):
        super().__init__()
        self.credentials = DataRobotDeploymentCredentials.model_validate(
            credentials, from_attributes=True
        )
    ...
    def _create_openai_client(self) -> AsyncOpenAI:
        ...
        if not self.chat_api_url:
            raise CustomModelLLMError(
                gettext("The URL of the deployment's chat endpoint was not specified.")
            )
        ...
        return AsyncOpenAI(
            base_url=base_url, api_key=api_key, timeout=DEFAULT_TIMEOUT.total, max_retries=0
        )
    ...
    async def _submit_prompt(
        ...
        if self.supports_chat_api and self.chat_api_url:
            return await self._submit_prompt_using_chat_api(
            ...
            return await self._submit_prompt_using_predictions_api(
                system_prompt=system_prompt,
                messages=messages,
                llm_settings=settings,
                docs=docs or [],
    ...
    async def _submit_prompt_with_response_streaming(
        self,
        messages: Sequence[ChatCompletionMessageParam],
        llm_settings: dict,
        docs: Sequence[Document] | None = None,
        ...
        """
        settings, system_prompt = get_settings_and_system_prompt(llm_settings)
        request_messages = prepend_system_message_to_messages(
            messages=messages, system_prompt=system_prompt, docs=docs or []
        )  # type: ignore[assignment]
        response_stream: AsyncIterator[
            ChatCompletionChunk
        ] = await make_openai_chat_api_call_to_custom_llm(
            client=self.client,
    ...
    async def _submit_prompt_using_chat_api(
        self,
        system_prompt: str,
        messages: Sequence[ChatCompletionMessageParam],
        llm_settings: dict,
        ...
        request_messages = prepend_system_message_to_messages(
            messages=messages, system_prompt=system_prompt, docs=docs
        )
        # Submit the request to the deployment.
        time_start = time()
        try:  # type: ignore[assignment]
            completion: ChatCompletion = await make_openai_chat_api_call_to_custom_llm(
                client=self.client,
                model=model_id or self.default_model_id,
                messages=request_messages,
                **llm_settings,
            ...
            if self.error_handler is not None:
                ...
                return BaseLanguageModelResult(
                    ...
                    result_metadata=BaseResultMetadata(
                        ...
                        error_message=gettext("Custom model LLM request returned an error."),
                        final_prompt=request_messages,
                    ),
                )
            else:
                ...
                msg = gettext(
                    "Custom model LLM request returned an error. Try again, and if that does not resolve the issue, send the prompt to the associated LLM blueprint and a more detailed error will be returned in the playground."
                )
    ...
    async def _submit_prompt_using_predictions_api(
        self,
        system_prompt: str,
        messages: Sequence[ChatCompletionMessageParam],
        llm_settings: dict,
        ...
        payload = construct_payload(
            self.input_type, [self.prompt_column_name], [input_text], self.association_id_column
        )
        # Submit the request to the deployed model
        time_start = time()
        try:
            async with ClientSession(headers=headers, timeout=DEFAULT_TIMEOUT) as session:
                response = await make_prompt_call_to_custom_llm(
                    session=session, inference_api_url=self.prediction_api_url, payload=payload
                )
