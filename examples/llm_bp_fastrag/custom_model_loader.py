...
def get_llm_credentials(
...
) -> LanguageModelCredentials | DataRobotDeploymentCredentials:
    ...
    if config.llm_credential_type == CustomModelLLMCredentialType.DATAROBOT:
        ...
        return DataRobotDeploymentCredentials(
            datarobot_key=config.external_custom_model_llm_settings.datarobot_key,
    ...
    credentials = LLMGatewayCredentials(
        base_url=client.endpoint.rstrip("/") + "/genai/llmgw",
        api_token=client.token,
        client_id=LLMWorkloads.CUSTOM_MODEL,
        llm_id=str(config.llm_id),
...
def load_llm_blueprint_model() -> LLMBlueprintModel:
    ...
    if (
        isinstance(credentials, DataRobotDeploymentCredentials)
        and config.external_custom_model_llm_settings is not None
    ):
        return LLMBlueprintModel(
            language_model=CustomModelLLM(
                credentials=credentials,
                prediction_api_url=config.external_custom_model_llm_settings.prediction_api_url,
                chat_api_url=config.external_custom_model_llm_settings.chat_api_url,
                model_type=config.external_custom_model_llm_settings.model_type,
                input_type=config.external_custom_model_llm_settings.input_type,
                prompt_column_name=config.external_custom_model_llm_settings.prompt_column_name,
                target_column_name=config.external_custom_model_llm_settings.target_column_name,
                association_id_column=config.external_custom_model_llm_settings.association_id_column,
                supports_chat_api=config.external_custom_model_llm_settings.supports_chat_api,
                chat_model_id=config.external_custom_model_llm_settings.chat_model_id,
            ),
            vector_store=vector_store,
        )  # type: ignore[arg-type]
    return LLMBlueprintModel(
        language_model=LLMGatewayLanguageModel(credentials), vector_store=vector_store
    )
