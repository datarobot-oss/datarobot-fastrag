...
@backoff.on_exception(
    backoff.expo,
    exception=(asyncio.TimeoutError, RetryableCustomModelError),
...
class CustomModelEmbeddingClient:
    ...
    def __init__(
        ...
        error_handler: BaseCustomModelEmbeddingErrorHandler | None = None,
        ...
        if reuse_session:
            self.session = ClientSession(
                headers=self.headers, raise_for_status=True, timeout=DEFAULT_TIMEOUT
            )
    ...
    def _construct_payload(self, texts: list[str], embedding_stage: EmbeddingStage) -> bytes:
        ...
        if self.input_type == "CSV":
            ...
            payload = json.dumps(
                {self.prompt_column_name: texts, "embedding_stage": embedding_stage}
            ).encode("utf8")
        return payload
    ...
    async def encode(self, sentences: list[str], embedding_stage: EmbeddingStage) -> np.ndarray:
        ...
        try:
            ...
            msg = gettext(
                "Custom model embedding request returned an error. Try again or check the custom model response in playground for additional details."
            )
            # We don't want to log the original exception as it could contain PII
...
class CustomModelEmbeddingError(Exception):
    """Raised when the custom model is unusable."""


class IncorrectResponseFromCustomModelError(Exception):
