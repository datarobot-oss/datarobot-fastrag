...
class DatasetExclusionList(DatasetAccessList):
    ...
    def get_filtering_condition(self) -> dict:
        return {
            MetadataColumnNames.source: {MetadataFilterOperators.NIN: list(self.files)},
            MetadataColumnNames.dataset_version: self.dataset_version_id,
        }
...
class DatasetInclusionList(DatasetAccessList):
    ...
    def get_filtering_condition(self) -> dict:
        return {
            MetadataColumnNames.source: {MetadataFilterOperators.IN: list(self.files)},
            MetadataColumnNames.dataset_version: self.dataset_version_id,
        }
...
class DatasetExcludeAll(DatasetAccessList):
    ...
    def get_filtering_condition(self) -> dict:
        return {
            MetadataColumnNames.dataset_version: {
                MetadataFilterOperators.NE: self.dataset_version_id
            }
        }
...
class AccessControlListLoader:
    ...
    def __init__(
        self,
        datasets_with_acl: list[tuple[str, str]],
    ...
    async def load_access_control_list(self) -> list[DatasetAccessList]:
        ...
        async with ClientSession(
            timeout=DEFAULT_TIMEOUT, headers=self.construct_headers()
        ) as session:  # type: ignore[arg-type]
            for dataset_id, dataset_version_id in self.datasets_with_acl:
                access_type, files = await self._load_all_access_control(
                    session, dataset_id, dataset_version_id
                )
                dataset_access_list = DatasetAccessList.create(
                    dataset_version_id, access_type, files
                )
    ...
    async def _load_all_access_control(
        self, session: ClientSession, dataset_id: str, dataset_version_id: str
    ) -> tuple[str, set[str]]:
        offset = 0
        etag = None
        access_type = None
        ...
        while total_count > len(all_files):
            access_type, files, etag, total_count = await self._load_access_control(
                session, dataset_id, dataset_version_id, offset=offset, etag=etag
            )
            offset += self.ACL_PAGE_LIMIT
            all_files.extend(files)
        return (access_type, set(all_files))  # type: ignore[return-value]

    async def _load_access_control(
        self,
        ...
        url = os.path.join(
            ...
            f"files/{dataset_id}/versions/{dataset_version_id}/accessibleFiles?limit={self.ACL_PAGE_LIMIT}&offset={offset}",
        )
        if etag:
            url += f"&etag={etag}"
...
class VectorStoreWithAccessControl(VectorStore):
    ...
    def __init__(
        ...
        authorization_header: str,
        datasets_with_acl: list[tuple[str, str]],
        datasets_without_acl: list[tuple[str, str]],
        has_legacy_datasets: bool = False,
        identity_token_loader: Callable | None = None,
    ):
        ...
        self.access_control_loader = AccessControlListLoader(
            datasets_with_acl, datarobot_endpoint, authorization_header, identity_token_loader
        )
    ...
    async def search(
        ...
        **kwargs: Any,
        ...
        documents, query_embeddings = await self.vector_store.search(
            query,
            k,
            self._update_filter(filter, acl_checker),
            add_neighbor_chunks,
            retrieval_mode,
    ...
    async def add_neighbor_chunks(
        ...
        docs: list[Document],
        filter: dict[str, Any] | None = DEFAULT_FILTER,
        apply_access_control_list: bool = False,
        **kwargs: Any,
    ) -> list[Document]:
        """
        ...
        if apply_access_control_list:
            acls = await self.access_control_loader.load_access_control_list()
            acl_checker = AccessControlChecker(acls=acls)
            documents = await self.vector_store.add_neighbor_chunks(
                docs, self._update_filter(filter, acl_checker), **kwargs
            )
    ...
    def _filter_without_acl(self) -> dict[str, dict[str, list]]:
        return {
            MetadataColumnNames.dataset_version: {
                MetadataFilterOperators.IN: self.datasets_without_acl
            }
        }

    def _update_filter(
        self, existing_filter: dict[str, Any] | None, acl_checker: AccessControlChecker
    ) -> dict[str, Any]:
