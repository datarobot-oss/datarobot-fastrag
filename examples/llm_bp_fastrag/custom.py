...
async def score(data: pd.DataFrame, model: LLMBlueprintModel, **kwargs: Any) -> pd.DataFrame:
    ...
    result_list, docs_list, provider_llm_guards_list, query_embeddings = await llm_blueprint_score(
        data, model, config
    )
...
async def chat(
    completion_create_params: CompletionCreateParams, model: LLMBlueprintModel, **kwargs: Any
