...
def ensure_required_runtime_parameter_set(parameter_name: str) -> None:
    ...
    if not parameter:
        raise CustomModelLLMError(
            f"{parameter_name} runtime parameter is required and cannot be empty"
        )


def load_openai_credentials() -> AzureOpenAICredentials:
    """Load OpenAI credentials from the runtime parameters of the model."""
    api_key = RuntimeParameters.get(CustomModelRuntimeParameters.OPENAI_API_KEY.value)
    if not api_key or "apiToken" not in api_key:
        raise CustomModelLLMError(
            'OPENAI_API_KEY runtime parameter is missing or invalid. Ensure that the parameter is assigned a credential of type "API Token" as its value.'
        )
    ...
    ensure_required_runtime_parameter_set(
        CustomModelRuntimeParameters.OPENAI_API_DEPLOYMENT_ID.value
    )
    credentials = AzureOpenAICredentials(
        api_type="azure",
        api_base=RuntimeParameters.get(CustomModelRuntimeParameters.OPENAI_API_BASE.value),
        api_version=RuntimeParameters.get(CustomModelRuntimeParameters.OPENAI_API_VERSION.value),
        deployment_id=RuntimeParameters.get(
            CustomModelRuntimeParameters.OPENAI_API_DEPLOYMENT_ID.value
        ),
        api_key=api_key["apiToken"],
    )
    return credentials
...
def load_google_credentials() -> GoogleVertexAICredentials:
    ...
    service_account = RuntimeParameters.get(
        CustomModelRuntimeParameters.GOOGLE_SERVICE_ACCOUNT.value
    )
    if not service_account or "gcpKey" not in service_account:
        raise CustomModelLLMError(
            'GOOGLE_SERVICE_ACCOUNT runtime parameter is missing or invalid. Ensure that the parameter is assigned a credential of type "Google Cloud Service Account" as its value.'
        )
    ...
    credentials = GoogleVertexAICredentials(
        region=RuntimeParameters.get(CustomModelRuntimeParameters.GOOGLE_REGION.value),
        service_account_info=service_account["gcpKey"],
    )
    return credentials
...
def load_amazon_credentials() -> AmazonBedrockCredentials:
    ...
    if not aws_account:
        raise CustomModelLLMError(
            'AWS_ACCOUNT runtime parameter is missing or invalid. Ensure that the parameter is assigned a credential of type "AWS" as its value.'
        )
    ensure_required_runtime_parameter_set(CustomModelRuntimeParameters.AWS_REGION.value)
    credentials = AmazonBedrockCredentials(
        access_key_id=aws_account["awsAccessKeyId"],
        secret_access_key=aws_account["awsSecretAccessKey"],
        session_token=aws_account.get("awsSessionToken"),
        region=RuntimeParameters.get(CustomModelRuntimeParameters.AWS_REGION.value),
    )
    return credentials
...
def load_anthropic_credentials() -> AnthropicCredentials:
    ...
    if not api_key or "apiToken" not in api_key:
        raise CustomModelLLMError(
            'ANTHROPIC_API_KEY runtime parameter is missing or invalid. Ensure that the parameter is assigned a credential of type "API Token" as its value.'
        )
...
def load_cohere_credentials() -> CohereCredentials:
    ...
    if not api_key or "apiToken" not in api_key:
        raise CustomModelLLMError(
            'COHERE_API_KEY runtime parameter is missing or invalid. Ensure that the parameter is assigned a credential of type "API Token" as its value.'
        )
...
def load_togetherai_credentials() -> TogetherAICredentials:
    ...
    if not api_key or "apiToken" not in api_key:
        raise CustomModelLLMError(
            'TOGETHERAI_API_KEY runtime parameter is missing or invalid. Ensure that the parameter is assigned a credential of type "API Token" as its value.'
        )
...
def load_groq_credentials() -> GroqCredentials:
    ...
    if not api_key or "apiToken" not in api_key:
        raise CustomModelLLMError(
            'GROQ_API_KEY runtime parameter is missing or invalid. Ensure that the parameter is assigned a credential of type "API Token" as its value.'
        )
...
def load_cerebras_credentials() -> CerebrasCredentials:
    ...
    if not api_key or "apiToken" not in api_key:
        raise CustomModelLLMError(
            'CEREBRAS_API_KEY runtime parameter is missing or invalid. Ensure that the parameter is assigned a credential of type "API Token" as its value.'
        )
