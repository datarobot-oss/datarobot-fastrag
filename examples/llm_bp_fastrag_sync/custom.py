...
def load_model(*args: Any, **kwargs: Any) -> LLMBlueprintModel:
    """Load the model object to be used in `score`. This custom model hook is called at runtime."""
    return load_llm_blueprint_model()


def score(data: pd.DataFrame, model: LLMBlueprintModel, **kwargs: Any) -> pd.DataFrame:
    config = load_custom_model_llm_blueprint_config()
    result_list, docs_list, provider_llm_guards_list, query_embeddings = llm_blueprint_score(
        data, model, config
    )
    ...
    populate_provider_llm_guard_columns(
        provider_llm_guards_list=provider_llm_guards_list, full_result_dict=full_result_dict
    )
    llm_blueprint_id_column_name = RuntimeParameters.get(
        CustomModelRuntimeParameters.LLM_BLUEPRINT_ID_COLUMN_NAME.value
    )
    if llm_blueprint_id_column_name is not None and RuntimeParameters.get(
        CustomModelRuntimeParameters.ENABLE_LLM_BLUEPRINT_ID_COLUMN.value
    ):
        full_result_dict[llm_blueprint_id_column_name] = [config.llm_blueprint_id] * len(
            result_list
        )
...
def chat(
    completion_create_params: CompletionCreateParams, model: LLMBlueprintModel, **kwargs
) -> ChatCompletion | Iterator[ChatCompletionChunk]:
    config = load_custom_model_llm_blueprint_config()
    return llm_blueprint_chat(model, config, completion_create_params)
