...
def translate_completion_create_params_to_llm_settings(
    ...
    system_prompt = next(
        (m for m in completion_create_params["messages"] if m["role"] == "system"), None
    )
    if system_prompt:
    ...
    return [
        {
            ...
            "link": f"{doc.metadata.get(MetadataColumnNames.source.value, '')}:{doc.metadata.get(MetadataColumnNames.page.value, '')}",
            "vector": doc.metadata.pop(MetadataColumnNames.doc_vector.value, None),
...
def create_chat_completion_from_llm_result(
    llm: LanguageModel,
    config: CustomModelLLMBlueprintConfig,
    llm_result: BaseLanguageModelResult,
    docs: Sequence[Document] | None = None,
    model_id: str | None = None,
    prompt_vector: QueryEmbeddings = None,
) -> ChatCompletion:
    ...
    if llm_result.tool_call_request:
        message["tool_calls"] = [
            ChatCompletionMessageToolCallParam(
                id=call.id,
                type="function",
                function=OpenAIFunction(name=call.tool_name, arguments=call.tool_arguments),
            )
            for call in llm_result.tool_call_request.tool_calls
        ]  # type: ignore[arg-type]
    choice = Choice(
        index=0, message=message, finish_reason="content_filter" if is_content_filtered else "stop"
    )
    ...
    completion = ChatCompletion(
        ...
        model=model_id or llm.default_model_id,
        usage=CompletionUsage(
            completion_tokens=llm_result.result_metadata.output_token_count,
            prompt_tokens=llm_result.result_metadata.input_token_count,
            total_tokens=llm_result.result_metadata.total_token_count,
        ),
    )
    ...
    if llm_result.result_metadata.provider_llm_guards:
        provider_guards = [
            guard.model_dump(exclude_none=False, exclude_unset=False)
            for guard in llm_result.result_metadata.provider_llm_guards
        ]
...
async def generate_chat_completion(
    ...
    llm_settings_from_request = translate_completion_create_params_to_llm_settings(
        completion_create_params
    )
    llm_settings = {}
    ...
    if use_streaming:
        ...
        return llm.submit_prompt_with_response_streaming(
            messages=messages,
            llm_settings=llm_settings,
            docs=docs,
            model_id=model_id,
            final_chunk_hook=attach_extra_metadata_to_final_chunk,
        )
    else:
        llm_result = await llm.submit_prompt(
            messages=messages, llm_settings=llm_settings, docs=docs, model_id=model_id
        )
        completion = create_chat_completion_from_llm_result(
            llm=llm,
            config=config,
