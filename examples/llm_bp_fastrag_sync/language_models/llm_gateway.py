...
class LLMGatewayLanguageModel(LanguageModel):
    ...
    def __init__(self, credentials: LLMGatewayCredentials):
        ...
        self.default_headers = {
            "User-Agent": "Buzok",
            "Client-Id": self.credentials.client_id.value,
        }
    ...
    def _create_request(
        ...
        extra_body = {
            "credential_json": self.credentials.user_credentials.model_dump_json(exclude_none=True)
            if self.credentials.user_credentials
            else None,
            "deployment_id": getattr(self.credentials.user_credentials, "deployment_id", None),
            "llm_id": None if model_id else self.credentials.llm_id,
            **settings,
        }
        request = {
    ...
    async def _submit_request(
        ...
        try:
            ...
            request_logger.warning(
                "LLM gateway chat completion errored", extra={"status_code": e.status_code}
            )
            error_detail = extract_error_detail(e)
    ...
    async def _submit_prompt(
        ...
        request = self._create_request(
            messages=messages, llm_settings=llm_settings, docs=docs, model_id=model_id, stream=False
        )
        time_start = time()
    ...
    async def _submit_prompt_with_response_streaming(
        ...
        request = self._create_request(
            messages=messages, llm_settings=llm_settings, docs=docs, model_id=model_id, stream=True
        )
...
def get_language_model_result_from_completion(
    completion: ChatCompletion, request_parameters: dict[str, Any], time_delta: float
) -> BaseLanguageModelResult:
    if completion.usage:
    ...
    if completion.choices[0].message.tool_calls:
        tool_call_request = BaseToolCallRequest(
            tool_calls=[
                BaseToolCall(
    ...
    return BaseLanguageModelResult(
        ...
        result_metadata=BaseResultMetadata(
            ...
            latency_milliseconds=int(time_delta * 1000),
        ),
    )
