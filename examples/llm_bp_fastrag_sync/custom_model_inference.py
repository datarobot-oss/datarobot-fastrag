...
from vector_database.connected_vector_store import ConnectedVectorStore
...
def get_external_embedding_client(
    external_embedding_model_parameters: ExternalDeploymentParameters,
) -> CustomModelEmbeddingClient:
    ...
    credentials = DataRobotDeploymentCredentials(
        datarobot_key=external_embedding_model_parameters.datarobot_key,
        authorization_header=get_datarobot_authorization_header(),
    )
    return CustomModelEmbeddingClient(
        credentials=credentials,
        prediction_api_url=external_embedding_model_parameters.prediction_api_url,
        model_type=external_embedding_model_parameters.model_type,
        input_type=external_embedding_model_parameters.input_type,
        prompt_column_name=external_embedding_model_parameters.prompt_column_name,
        target_column_name=external_embedding_model_parameters.target_column_name,
    )
...
def load_stored_vector_database_as_vector_store(
    config: CustomModelLLMBlueprintConfig,
) -> VectorStore | None:
    ...
    os.environ["DEVICE_FOR_NEURAL_NETWORK_COMPUTATIONS"] = RuntimeParameters.get(
        CustomModelRuntimeParameters.DEVICE_FOR_NEURAL_NETWORK_COMPUTATIONS.value
    )
    ...
    if config.vector_database_settings.external_vector_database_parameters is None:
        if config.vector_database_settings.external_embedding_model_parameters:
            client: SentenceTransformer | CustomModelEmbeddingClient = (
                get_external_embedding_client(
                    config.vector_database_settings.external_embedding_model_parameters
                )
            )
        else:
            client = get_sentence_transformer(
                VectorDatabaseCustomModelObjects.EMBEDDING_MODEL.value
            )
        ...
        if not external_params.prompt_column_name:
            raise RuntimeError(
                "Prompt column name is not set for the custom model vector database."
            )
        if not external_params.target_column_name:
            raise RuntimeError(
                "Target column name is not set for the custom model vector database."
            )
        prediction_api_url = _handle_prediction_api_url_substitutions(external_params)
        vector_store = CustomModelVectorDatabaseAsVectorStore(
            prediction_api_url=prediction_api_url,
            datarobot_key=external_params.datarobot_key,
            authorization_header=get_datarobot_authorization_header(),
            model_type=external_params.model_type,
            input_type=external_params.input_type,
            prompt_column_name=external_params.prompt_column_name,
            target_column_name=external_params.target_column_name,
            association_id_column=external_params.association_id_column,
            identity_token_loader=get_identity_token,
        )
    return vector_store


def _handle_prediction_api_url_substitutions(
    external_params: ExternalDeploymentParameters | None,
) -> str:
    pred_api_url_overridden_deployment_id = _substitute_vdb_deployment_runtime_parameter(
        external_params
    )
    final_prediction_api_url = _substitute_prediction_api_url_for_nginx(
        pred_api_url_overridden_deployment_id
    )
    return final_prediction_api_url


def _substitute_vdb_deployment_runtime_parameter(
    external_params: ExternalDeploymentParameters | None,
) -> str:
    vector_database_deployment_id_param = RuntimeParameters.get(
        CustomModelRuntimeParameters.VECTOR_DATABASE_DEPLOYMENT_ID.value
    )
    if not vector_database_deployment_id_param:
        return external_params.prediction_api_url if external_params else ""
    prediction_api_components = (
        external_params.prediction_api_url.split("/") if external_params else []
    )
    prediction_api_url = (
        "/".join(prediction_api_components[:-2])
        + "/"
        + vector_database_deployment_id_param
        + "/"
        + "/".join(prediction_api_components[-1:])
    )
...
def _substitute_prediction_api_url_for_nginx(prediction_api_url: str) -> str:
    ...
    if os_datarobot_endpoint and os_datarobot_endpoint.startswith(
...
def get_excess_token_budget_for_custom_model_llm_blueprint_config(
    config: CustomModelLLMBlueprintConfig, user_prompt: str
) -> int:
    ...
    context_size = (
        config.llm_settings.get("external_llm_context_size", config.context_size)
...
def create_retriever_for_llm_blueprint(
    model: LLMBlueprintModel, config: CustomModelLLMBlueprintConfig
) -> BaseRetriever | None:
    """
    ...
    if model.vector_store and config.vector_database_settings:
        retriever = get_retriever(
            retriever_name=config.vector_database_settings.retriever,
            vector_store=model.vector_store,
            language_model=model.language_model,
            llm_settings=config.llm_settings,
            number_of_documents_to_retrieve=config.vector_database_settings.number_of_documents_to_retrieve,
            add_neighbor_chunks=config.vector_database_settings.add_neighbor_chunks,
        )
    return retriever


async def retrieve_and_prune_docs(
    query: str | Iterable[ChatCompletionContentPartParam],
    ...
    docs, rewritten_query, rewritten_history, query_emb = await retriever.get_docs_and_query(
        query=str(query), history=history, metadata_filter=metadata_filter
    )
    docs_token_budget = get_excess_token_budget_for_custom_model_llm_blueprint_config(
        config=config, user_prompt=rewritten_query
    )
...
def get_non_english_tokenizer_from_retriever(
    ...
    def _is_non_english(tokenizer: PreTrainedTokenizerBase) -> bool:
        return any(
            (
                str(model) in tokenizer.name_or_path
                for model in CustomModelNonEnglishEmbeddingModelPaths
            )
        )
...
async def submit_llm_blueprint_prompt(
    model: LanguageModel,
    query: str,
    config: CustomModelLLMBlueprintConfig,
    retriever: BaseRetriever | None = None,
    metadata_filter: dict[str, Any] | None = None,
) -> tuple[BaseLanguageModelResult, list[Document], QueryEmbeddings]:
    """
    ...
    docs, rewritten_query, _, query_emb = await retrieve_and_prune_docs(
        query=query, history=[], config=config, retriever=retriever, metadata_filter=metadata_filter
    )
    messages: list[ChatCompletionMessageParam] = [
        ChatCompletionUserMessageParam(role="user", content=rewritten_query)
    ]
...
async def generate_llm_blueprint_chat_completion(
    model: LanguageModel,
    config: CustomModelLLMBlueprintConfig,
    ...
    docs, rewritten_query, rewritten_history, query_emb = await retrieve_and_prune_docs(
        query=next_user_prompt["content"],
        history=history_before_next_user_prompt,
        config=config,
        retriever=retriever,
    ...
    result = await generate_chat_completion(
        ...
        docs=docs,
        prompt_vector=query_emb,
    )
    return result


def parse_metadata_filter(
    data: pd.DataFrame, idx: int, config: CustomModelLLMBlueprintConfig
) -> dict[str, str] | None:
    ...
    if config.vector_database_settings.external_vector_database_parameters:
        return filter_dict
    allowed_keys = set(
        config.vector_database_settings.enabled_citations_fields + MetadataFilterOperators.all()
    )
    for k in filter_dict:
        if k not in allowed_keys:
            raise ValueError(
                f"Metadata filter in row {idx} uses disallowed column or operator '{k}'"
            )
    return filter_dict


def llm_blueprint_score(
    data: pd.DataFrame, model: LLMBlueprintModel, config: CustomModelLLMBlueprintConfig
) -> tuple[
    list[str | None],
    list[list[Document]],
    list[list[BaseProviderGuardsMetadata] | None],
    ...
    prompt_column_name = RuntimeParameters.get(
        CustomModelRuntimeParameters.PROMPT_COLUMN_NAME.value
    )
    ...
    for idx, query in enumerate(data[prompt_column_name].tolist()):
        ...
        language_model_result, docs, query_emb = get_coroutine_result(
            submit_llm_blueprint_prompt(
                model.language_model, query, config, retriever, metadata_filter
            )
        )
...
def llm_blueprint_chat(
    model: LLMBlueprintModel,
    config: CustomModelLLMBlueprintConfig,
    completion_create_params: CompletionCreateParams,
    ...
    result = get_coroutine_result(
        generate_llm_blueprint_chat_completion(
    ...
    if isinstance(result, AsyncIterator):
        return async_to_sync_iterator(result)
    else:
        return result


def populate_provider_llm_guard_columns(
    provider_llm_guards_list: list[list[BaseProviderGuardsMetadata] | None],
    full_result_dict: dict[str, list],
) -> None:
    unique_metrics = set()
...
def async_to_sync_iterator(async_iter: AsyncIterator[T]) -> Iterator[T]:
    """
    Convert a lazy asynchronous iterator into a lazy synchronous iterator.

    The conversion is based on consuming items from the async iterator in a background thread
    and sending them to the calling (main) thread using a queue. The calling thread reads the items
    from the queue and yields them from a synchronous iterator.

    Parameters
    ----------
    async_iter
        The async iterator to consume from.

    Yields
    ------
    Items consumed from the async iterator.
    """
    # A synchronized queue to feed data from the thread iterating over the async generator
    ...
    done_marker = object()
    ## flask_app = flask.current_app._get_current_object() if flask.has_app_context() else None  # type: ignore[attr-defined]
    flask_app = None
    # The entry point of the async consumption thread.

    def run_async_consumer() -> None:
        # Push Flask app context into async consumer thread if we have an app instance with context.
        # This allows to access DRUM's background event loop from consumer thread
...
def populate_monitoring_columns(
    full_result_dict: dict[str, list],
    ...
    enabled_citation_columns = json.loads(
        RuntimeParameters.get(CustomModelRuntimeParameters.ENABLE_CITATION_COLUMNS.value)
    )
    citation_fields = (
        config.vector_database_settings.enabled_citations_fields
        if config.vector_database_settings
        and config.vector_database_settings.enabled_citations_fields
        else MetadataColumnNames.custom_model_citation_fields()
    )
            ...
            for citation_field in citation_fields:
                if not enabled_citation_columns.get(citation_field, False):
                    if citation_field in llm_context_item:
                        ...
                        "metadata" in llm_context_item
                        and citation_field in llm_context_item["metadata"]
                    ):
                        llm_context_item["metadata"].pop(citation_field)
        full_result_dict["_LLM_CONTEXT"][i] = json.dumps(llm_context_list)


def get_drum_background_loop() -> AbstractEventLoop | None:
    """Get a background event loop created in DRUM."""
    # try:
    #     if hasattr(flask.current_app, "_drum_bg_loop"):
    #         loop = flask.current_app._drum_bg_loop
    #         if loop is not None and loop.is_running():
    #             return loop
    # except RuntimeError as e:
    #     logger.bind(message=str(e)).warning("RuntimeError when getting DRUM background loop")
    return None


def get_coroutine_result(coro: Coroutine[Any, Any, T]) -> T:
    """
    Execute a coroutine in a background loop if it's available and return result.
    If a background loop is not available, fallback to `asyncio.run` which creates its own loop
    """
    background_loop = get_drum_background_loop()
    if background_loop is not None:
        future = asyncio.run_coroutine_threadsafe(coro, background_loop)
        try:
            return future.result(timeout=5 * 60)
